...
export default function RnDPage() {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Yakin ingin menghapus resep ujicoba ini?')) {
  ...
  const createNew = async () => {
    try {
      const res = await axios.post('/api/rnd', {
        name: 'Resep Ujicoba Baru',
        description: '',
        status: 'Draft'
      });
      window.location.href = `/admin/rnd/${res.data.id}`;
    } catch(e) {
      alert("Gagal membuat resep baru");
    }
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-zinc-900 flex items-center gap-2">
            ...
            R&D Eksperimen Resep
          </h1>
          <p className="text-zinc-500 text-sm mt-1">Ujicoba resep baru dan hitung simulasi HPP</p>
        </div>
      ...
      <Card className="p-4 mb-6">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Cari resep ujicoba..."
            value={searchQuery}
